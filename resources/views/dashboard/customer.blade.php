...
@section('dash.main')
    <div class="row justify-content-end">
        <a href="{{ route('customer.create') }}" role="button" class="btn btn-dark col col-auto"> <i
    ...
    <div class="my-4">
        ...
        <form action="{{ route('customer.destroy') }}" method="POST">
            ...
            <div class="my-3 text-end">
                <button type="submit" class="btn btn-danger">Supprimer les Clients Selectionner</button>
            </div>
            <h2 class="text-fv-primary my-2"> {{ $posts->count() }} Clients </h2>
            <ul class="list-group">
                @foreach($posts as $post)
                    <li class="list-group-item list-group-item-action">
                        <input type="checkbox" name="{{ $post->id }}" class="form-check-input">
                        <a href="{{ route('customer.edit', $post) }}"
