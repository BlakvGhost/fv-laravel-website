...
@section('title')
    Voir les Partenaire | Admin
@endsection
@section('dash.main')
    <div class="row justify-content-end">
        <a href="{{ route('partner.create') }}" role="button" class="btn btn-dark col col-auto"> <i
    ...
    <div class="my-4">
        ...
        <form action="{{ route('partner.destroy') }}" method="POST">
            ...
            <h2 class="text-fv-primary my-2"> {{ $posts->count() }} partenaires </h2>
            <ul class="list-group">
                @foreach($posts as $post)
                    <li class="list-group-item list-group-item-action">
                        <input type="checkbox" name="{{ $post->id }}" class="form-check-input">
                        <a href="{{ route('partner.edit', $post) }}"
