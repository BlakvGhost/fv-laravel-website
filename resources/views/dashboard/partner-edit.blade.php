...
@section('dash.main')
    ...
    <form action="{{ route('partner.update', $post) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if( $errors->any())
        ...
        <div class="form-group my-2">
            ...
            <input type="text" name="name" value="{{ $post->name }}" id="title" placeholder="Entrer le Nom"
                   class="form-control">
        </div>
        <div class="form-group my-2">
            <label for="title2" class="form-text">Classe mdi du Logo </label>
            <input type="text" name="icon" value="{{ $post->icon }}" id="title2" placeholder="Entrer le nom du Logo"
                   class="form-control">
        </div>
        <div class="form-group my-2">
            <label for="cover" class="form-text">Logo </label>
            <input type="file" name="logo" id="cover"
                   class="form-control">
            @if($post->logo)
